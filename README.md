...
used.system
 ...
 ├── controller
 │    ├── like        # 찜 API
 │    ├── member      # 회원가입·로그인/로그아웃 API, SessionConst, 요청/응답 DTO
...
요청 본문만 보고 판정되면 어노테이션, 저장소를 봐야 알면 서비스다.

비밀번호 확인(`passwordConfirm`)은 **서버가 받지 않는다.** 오타 방지용 UX 장치라 서버가 지킬 무결성이
아니고 — 우회해도 본인이 오타 난 비밀번호를 쓰게 될 뿐이다 — 평문 비밀번호를 두 번 실어 보내면
로그에 남을 표면만 늘어난다. 입력칸 두 개를 비교하는 일은 화면을 붙일 때 그쪽에서 한다.
