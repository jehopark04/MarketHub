...
- 보는 사람에 따라 달라지는 값(`liked` 같은)은 **도메인이 아니라 응답 DTO에** 둔다.
  그 판정 규칙("비로그인은 찜이 없는 것으로 친다")은 서비스 한 곳에만 두고, 목록과
  단건이 나눠 쓴다 — 컨트롤러마다 다시 내리면 화면이 늘 때마다 복사된다.
...
- 페이지 주소는 **확장자 없이** 쓴다(`/login`). 스프링은 정적 파일을 파일 이름 그대로만
  매핑하므로, 새 페이지를 만들면 `WebConfig.addViewControllers`에 `forward` 한 줄을
  더해야 한다. 빠뜨리면 그 주소가 404다.

### 인증 (2단계에서 정함)
- **`401`을 어떻게 다룰지는 요청마다 다르다.** `api.js`의 `unauthorized` 옵션으로 정한다.
  - `'redirect'`(기본) — 로그인 화면으로 보낸다. 로그인이 필요한 동작이었다는 뜻.
  - `'throw'` — 401이 오류가 아니라 **답**인 요청에 쓴다. 내 정보 조회의 401은
    "비로그인"이고, 로그인 요청의 401은 "아이디나 비밀번호가 틀렸다"다.
    이 둘을 로그인 화면으로 보내면 비로그인 방문자가 목록만 보러 왔다가 튕기고,
    로그인 화면은 자기 자신으로 되돌아 무한히 돈다.
- 돌아갈 곳(`?redirect=`)은 **`safeRedirect`로 거른다.** 값을 그대로 믿고 이동하면
  외부 주소로 튕겨 보낼 수 있다(오픈 리다이렉트). `//evil.example`도 밖으로 나가므로
  앞글자만 보지 말고 실제 주소로 해석해 출처를 비교한다.
- **헤더는 `session.js`가 그린다.** 페이지마다 HTML을 복붙하면 로그인 표시를 고칠 때
  전부 손봐야 한다.
- 로그인 여부가 필요하면 **`renderHeader()`의 반환값**을 쓴다. 따로 `fetchMe`를 부르면
  같은 정보를 두 번 물어 `/api/me`가 페이지당 두 번 나간다.
- 검증 실패(`400`)의 `errors` 키는 **입력칸 `name`과 같다.** 그대로 찾아 넣으면 된다.
  특정 칸의 것이 아닌 실패(`409`·`401`)는 폼 위쪽에 한 줄로 보여준다.
- 제출하는 동안 **버튼을 잠근다.** 연타로 같은 요청이 두 번 나가지 않게.

### 목록에서 상태 바꾸기 (3단계에서 정함)
- **낙관적으로 먼저 바꾸고 실패하면 되돌린다.** 응답을 기다렸다가 바꾸면 눌렀는데
  반응이 없는 순간이 생긴다.
- **요청이 끝나기 전 다시 누르면 무시한다.** 서버가 멱등이라 견디더라도, 응답이 엇갈려
  도착하면 화면이 실제 상태와 어긋난다.
- 다시 그려지는 요소의 이벤트는 **컨테이너에 위임한다.** 항목마다 리스너를 달면 목록을
  그릴 때마다 쌓인다.
- `404`는 그 대상이 사라진 것이다. 되돌리는 데 그치지 말고 **화면에서도 뺀다** —
  남겨두면 눌러도 계속 실패한다.
- 실패 알림은 `showToast`를 쓴다. **`alert()`를 쓰지 않는다** — 페이지를 멈춰 세우고
  브라우저 기본 모양이라 화면과 겉돈다.
- 누르는 것은 `<button>`으로 만들고 상태는 `aria-pressed`로 알린다. 모양만 바꾸면
  키보드로 닿지 않고 낭독기가 읽지 못한다.
- 여러 화면이 쓰는 동작은 공용 모듈로 뺀다(`js/likes.js`). 다만 **실패했을 때 할 일은
  화면마다 다르므로** 공용 쪽이 정하지 않고 콜백으로 받는다 — 목록은 그 카드를 빼고,
  상세는 목록으로 돌려보낸다. 공용 모듈이 정해버리면 화면이 늘 때마다 분기가 쌓인다.
- 카드 전체를 링크로 만들 때 **카드를 `<a>`로 감싸지 않는다.** 안에 있는 하트를 눌러도
  링크가 따라간다. 제목 링크의 `::after`로 카드를 덮고, 하트를 `z-index`로 위에 올린다.
- **소유자에 따라 버튼을 감추는 것은 편의일 뿐 방어가 아니다.** 숨겨도 남의 것에 요청을
  쏘면 서버가 `403`으로 막는다. 그 `403`도 화면에서 처리하고, 폼을 열 때도 소유자가
  아니면 되돌려보낸다 — 다 채운 뒤 실패를 알리는 것보다 낫다.
- 되돌릴 수 없는 동작은 한 번 묻되 **`confirm()`을 쓰지 않는다.** `<dialog>`를 쓴다 —
  Esc로 닫히고 초점이 안에 갇히는 것을 브라우저가 해준다.
- 같은 입력을 다루는 폼(등록·수정)은 **한 곳에서 다룬다.** 다른 것은 기존값을 채우는가,
  어느 메서드인가, 성공 후 어디로 가는가뿐이다. 두 벌로 두면 검증 메시지나 필드를
  한쪽만 고치는 사고가 난다.
- 숫자 입력이 빈 칸이면 **`null`로 보낸다.** 빈 문자열을 `Number()`에 넣으면 `0`이 되어
  "공짜"가 등록된다.
- 상세처럼 id가 필요한 화면은 `/products/1`로 열고 `location.pathname`에서 읽는다.
  `WebConfig`의 매핑이 `forward`라 쿼리로 오지 않는다. 매핑에 **숫자 제약을 둔다** —
  없으면 `/products/new` 같은 경로가 상세로 걸린다.

> ⚠️ `bootRun`은 `build/resources/main/`에서 서빙한다. 실행 중에 `src/`의 정적 파일을
> 고쳐도 반영되지 않으니, 화면을 고쳤으면 **앱을 다시 띄운다.**
