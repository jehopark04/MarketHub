...
- 숫자 입력이 빈 칸이면 **`null`로 보낸다.** 빈 문자열을 `Number()`에 넣으면 `0`이 되어
  "공짜"가 등록된다.
- 상품 카드는 `js/product-card.js` 하나로 그린다. 목록·내 상품·찜 목록이 같은 필드를
  쓴다 — 따로 그리면 가격 표기나 링크 주소를 한쪽만 고치는 사고가 난다.
  화면마다 다른 것(하트를 그릴지)은 인자로 받는다.
- 탭처럼 보이는 화면도 **주소를 각각 갖는다**(`/my-page/likes`). 상태를 자바스크립트
  변수로만 들고 있으면 뒤로 가기가 동작하지 않고 링크를 공유할 수 없다.
  뼈대가 같으면 한 HTML로 `forward`하고 `pathname`으로 갈라도 된다.
- 목록에서 **되돌릴 수 있는 동작은 항목을 즉시 빼지 않는다.** 찜 목록에서 찜을 풀면
  빈 하트로 남겨 다시 누를 수 있게 하고, 사라지는 것은 새로고침에 맡긴다.
  대상 자체가 없어진 경우(`404`)만 즉시 뺀다.
